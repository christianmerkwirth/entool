...
ternary_search_tree<KEY, Evaluater>::ternary_search_tree(const long keylength) : bufn(0), freen(0), 
	root(0), next_buf_size(TST_BUFSIZE), old_buf_size(0), len(keylength), nr_total_nodes(0) {}
...
{   
    ...
    for (;;) {	/* once we find a node that is not allocated (==0), we must create every next node */
		if (freen-- == 0) {
			...
			next_buf_size += old_buf_size; 	// fibonacci like increasing of buffer size (this keeps overall number of allocations small)
...
{   
	...
	long buf_size = TST_BUFSIZE;		// the actual size of the buffer is increasing each iteration
